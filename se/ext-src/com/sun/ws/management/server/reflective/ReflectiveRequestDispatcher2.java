...
public final class ReflectiveRequestDispatcher2 extends WSManRequestDispatcher {
    ...
    public ReflectiveRequestDispatcher2(final WSManagementRequest req, final WSManagementResponse resp,
    ...
    public WSManagementResponse call() throws Exception {
        final String resource = getRequest().getResourceUri().toString();
        if (resource == null) {
        ...
        final String action = getRequest().getActionURI().toString();
        try {
            
            // XXX REVISIT THIS IS AT THIS POINT THAT YOU NEED TO CHANGE HANDLER API!!!!!
            he.getMethod().invoke(he.getInstance(), action, resource, getContext(),
