...
public class WSEventingSupportSinkTest extends TestBase {
	...
	public void testcreateDeliveryRefusedFaultMessage()  {
		try{
			...
			final Addressing response = WSEventingSupportSink.createDeliveryRefusedFaultMessage(request, Faultreason);  
			System.out.println(response);
			assertEquals(Management.FAULT_ACTION_URI,response.getAction());     //test action
			assertNotNull(response.getMessageId());                        //test messageID
			assertEquals(request.getMessageId(),response.getRelatesTo()[0].getValue());           //test relatesto                          
			assertEquals(SOAP.RECEIVER, response.getBody().getFault().getFaultCodeAsQName());       //test faultcode
			assertEquals(DeliveryRefusedFault.DELIVERY_REFUSED, response.getFault().getCode().getSubcode().getValue());  //test subcode
			assertEquals(Faultreason,response.getFault().getReason().getText().get(0).getValue());   //test reason
		}
		catch(Exception e){
			e.printStackTrace();
		} 
	...
	//Construct the request message
		...
		request.getEnvelope().addNamespaceDeclaration(SOAP.NS_PREFIX, SOAP.NS_URI);
		...
		final Element temperature = tempDoc.createElementNS(SOAP.NS_URI, SOAP.NS_PREFIX + ":" + "temperature");
		...
		final Element units = unitsDoc.createElementNS(SOAP.NS_URI, SOAP.NS_PREFIX + ":" + "units");
		units.setAttributeNS(SOAP.NS_URI, SOAP.NS_PREFIX + ":" + "type", "celsius");
		...
		final QName portType = new QName(SOAP.NS_URI, "thePortType", SOAP.NS_PREFIX);
		...
		final QName serviceName = new QName(SOAP.NS_URI, "theServiceName", SOAP.NS_PREFIX);
		...
		final Addressing response = WSEventingSupportSink.createEventACKAcknowledgement(request);
		System.out.println(response);
		response.prettyPrint(logfile);
		final ByteArrayOutputStream bos = new ByteArrayOutputStream();
		response.writeTo(bos);
    
		assertEquals(response.getAction(),"http://schemas.dmtf.org/wbem/wsman/1/wsman/Ack");
		assertEquals(response.getTo(),request.getReplyTo().getAddress().getValue());
		assertEquals(response.getRelatesTo()[0].getValue(),request.getMessageId());
